#' Plot summary of evaluation results.
#' 
#' @description General helper function to plot summary of evaluation results
#'   as a boxplot, scatter plot, line plot, or bar plot with or without 1 SD
#'   error bars.
#' 
...
#' @param evaluator_name (Optional) Name of evaluator containing results to 
#'   plot. If not provided, evaluation results will be automatically
#'   computed by calling \code{eval_fun()}. If the evaluation results have
#'   already been computed previously, should provide \code{evaluator_name} to
#'   avoid duplicate computations.
#' @param eval_out (Optional) Tibble containing evaluation results to plot. If
#'   provided, \code{eval_results} and \code{evaluator_name} are ignored, and 
#'   the tibble given by \code{eval_out} is used for plotting instead. This 
#'   argument is useful if you need to overwrite the tibble given by
#'   \code{eval_results[[evaluator_name]]} with something else.
#' @param eval_id Character string. ID used for naming columns in
#'   \code{summarize_eval_results()}. Should be the same as the \code{id}
#'   input argument in \code{summarize_eval_results()}.
...
#'   been computed in \code{eval_results[[evaluator_name]]}.
#' @param vary_params Vector of parameter names that are varied in the 
#'   \code{Experiment}'s run.
#' @param show Character vector with elements being one of "boxplot", "point",
#'   "line", "bar", "errorbar", "ribbon" indicating what plot(s) to construct.
#' @param x_str (Optional) Name of column in evaluation results data frame to
#'   plot on the x-axis.
#' @param y_str (Optional) Name of column in evaluation results data frame to 
#'   use for plotting on the y-axis if \code{show} is anything but "boxplot".
#' @param err_sd_str (Optional) Name of column in evaluation results data frame
#'   containing the standard deviations of \code{y_str}.
#' @param color_str (Optional) Name of column in evaluation results data frame 
#'   to use for the color aesthetic when plotting.
...
#'   if need be.
#' @param plot_by (Optional) Name of column in evaluation results data frame
#'   to use for subsetting and creating different plots for each unique value.
#' @param add_ggplot_layers Additional layers to add to a ggplot object via 
#'   \code{+}.
#' @param boxplot_args (Optional) Additional arguments to pass to 
#'   \code{ggplot2::geom_boxplot()}.
#' @param point_args (Optional) Additional arguments to pass to 
#'   \code{ggplot2::geom_point()}.
#' @param line_args (Optional) Additional arguments to pass to 
#'   \code{ggplot2::geom_line()}.
#' @param bar_args (Optional) Additional arguments to pass to 
#'   \code{ggplot2::geom_bar()}.
#' @param errorbar_args (Optional) Additional arguments to pass to 
#'   \code{ggplot2::geom_errorbar()}.
#' @param ribbon_args (Optional) Additional arguments to pass to
#'   \code{ggplot2::geom_ribbon()}.
#' @param facet_args (Optional) Additional arguments to pass to 
#'   \code{ggplot2::facet_grid()} or \code{ggplot2::facet_wrap()}.
#' @param interactive Logical. Whether or not to return interactive
#'   \code{plotly} plots instead of static \code{ggplot} plots.
#' @param ... Additional arguments to pass to \code{eval_fun()}.
#' 
#' @return A ggplot object or list of ggplot objects.
#' 
#' @export
plot_eval_summary <- function(fit_results, eval_results, evaluator_name = NULL, 
                              eval_out = NULL, eval_id = "", 
    ...
    if ("bar" %in% show) {
      plt <- plt +
        do.call(ggplot2::geom_bar,
                args = c(list(mapping = base_aes, stat = "identity"), bar_args))
    }
    if ("boxplot" %in% show) {
      if (is.null(color_str)) {
        group_str <- x_str
