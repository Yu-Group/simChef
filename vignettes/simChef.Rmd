...
# Simulation Experiment Library

In addition to the simple grammar, `simChef` provides a built-in library of functions to create commonly used DGPs, Evaluators, and Visualizers for simulation experiments. The full library of functions can be viewed [here](https://yu-group.github.io/simChef/reference/index.html#section-simulation-experiment-library).

## Simulation Experiment Templates

We have also provided boilerplate code templates for running common *types* of simulation experiments, namely, those focused on prediction (regression and classification), feature selection, or inference. These templates both (1) provide a quick starting point with Evaluators and Visualizers that are commonly used for the specified type of simulation experiment and (2) provide a concrete example of how to get started using functions from the `simChef` library.
...
TODO: using the developer functions in the library to create custom functions
...
```{r eval = FALSE}
results <- experiment$run(n_reps = 20, use_cached = TRUE, save = TRUE)
get_cached_results(experiment)
```
...
There are also several customizable options regarding the aesthetics of Evaluators and Visualizers displayed in the Rmd report. These can be modified using the `rmd_options` argument in `create_evaluator()` and `create_visualizer()`. For example, we can customize the height and width of the Power plot via
```{r}
power_plot <- create_visualizer(viz_fun = power_plot_fun, 
...
If working interactively, the error can be inspected using the usual call to `rlang::last_error()`, and results that were run and completed before the error can be inspected via `rlang::last_error()$partial_results`. 
