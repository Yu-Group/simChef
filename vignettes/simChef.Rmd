...
### Step 1. Specify/write DGP, method, evaluation, and visualization functions of interest. {-}
...
<!-- Thus, focusing on step 1, it is important to understand (1) at a high-level, how `DGPs`, `Methods`, `Evaluators`, and `Visualizers` play together when running an `Experiment` via `run_experiment()` and (2) the inputs and outputs that are necessary for writing each user-defined function. We discuss these two points in turn. -->
...
```{r run-experiment-figure, fig.cap = "Overview of running a `simChef` `Experiment`. The `Experiment` class handles relationships among the four classes: `DGP`, `Method`, `Evaluator`, and `Visualizer`. Experiments may have multiple `DGPs` and `Methods`, which are combined across the Cartesian product of their varying parameters (represented by *). Once computed, each `Evaluator` and `Visualizer` takes in the fitted simulation replicates, while `Visualizer` additionally receives evaluation summaries.", out.width = "100%", echo = FALSE}
knitr::include_graphics(path = "figures/run_experiment.png")
```
...
```{r fit-experiment-figure, fig.cap = "Overview of fitting a `simChef` `Experiment`. Inputs and outputs for user-defined functions are also provided.", out.width = "100%", echo = FALSE}
knitr::include_graphics(path = "figures/fit_experiment.png")
```
...
```{r eval-experiment-figure, fig.cap = "Overview of evaluating a `simChef` `Experiment`. Inputs and outputs for user-defined functions are also provided. Dotted boxes denote optional input arguments.", out.width = "100%", echo = FALSE}
knitr::include_graphics(path = "figures/eval_experiment.png")
```
...
```{r viz-experiment-figure, fig.cap = "Overview of visualizing a `simChef` `Experiment`. Inputs and outputs for user-defined functions are also provided. Dotted boxes denote optional input arguments.", out.width = "100%", echo = FALSE}
knitr::include_graphics(path = "figures/viz_experiment.png")
```

## Putting it all together {-}

As alluded to previously, once the `DGP`, `Method`, `Evaluator`, and `Visualizer` functions have been defined, the hard work has been done. For convenience, we provide below a template that can be used to run an experiment, assuming that `dgp_fun`, `method_fun`, `eval_fun`, and `viz_fun` have been appropriately defined.
...
eval <- create_evaluator(
  .eval_fun = eval_fun, .name = "Evaluator Name",
  # additional named parameters to pass to .eval_fun()
  eval_param1 = XXX, eval_param2 = XXX, ...
)
viz <- create_visualizer(
  .viz_fun = viz_fun, .name = "Visualizer Name",
  # additional named parameters to pass to .viz_fun()
...
- [simChef culinary school (tutorial slides)](https://jpdunc23.github.io/simChef-culinary-school/slides/slides.html)
- [Computing experimental replicates in parallel (vignette)](`parallel.html`)
- [Setting up your simulation study (vignette)](`simulation-directory-setup.html`)
- [simChef: A comprehensive guide (vignette)](`simChef-full.html`) for information on caching, checkpointing, error debugging, common simulation experiment templates, and more
