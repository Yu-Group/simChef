...
By default, all `Visualizers` in the built-in `simChef` library use the default `ggplot` theme when plotting. To modify this plotting theme (for all built-in `Visualizers` and any `Visualizer` built upon `plot_eval_summary()`), one can change the `simChef.plot_theme` global option.

For instance, to use the [vthemes](https://yu-group.github.io/vthemes/) plotting theme:

```{r eval = FALSE}
options(simChef.plot_theme = "vthemes")  # to use the vthemes plotting theme (see)
```
