...
# copy html output to vignettes/results/example-html-outputs folder for website
if (!dir.exists(file.path("results", "example-html-outputs"))) {
  dir.create(file.path("results", "example-html-outputs"), recursive = TRUE)
}
file.copy(from = file.path(experiment$get_save_dir(), 
                           paste0(experiment$name, ".html")),
          to = file.path("results", "example-html-outputs", 
                         "linear_regression_base_output.html"),
          overwrite = TRUE)
```
...
Given that the `simChef` simulation library cannot possibly cover all possible simulation scenarios, we have made available a library of developer functions that can assist writing custom DGP, Evaluator, and Visualizer functions. These functions can be found [here](https://yu-group.github.io/simChef/reference/index.html#section-developer-functions). A vignette with concrete examples on how to write custom DGP, Evaluator, and Visualizer functions using this library is in the works. Stay tuned!
