...
author: "James Duncan, Corrine F. Elliott, Tiffany M. Tang"
...
Once a DGP, Method, Evaluator, or Plotter has been added to the simulation experiment recipe, the component can be updated using `update_dgp()`, `update_method()`, `update_evaluator()`, or `update_plotter()` and removed using `remove_dgp()`, `remove_method()`, `remove_evaluator()`, or `remove_plotter()`.
...
The experiment can also be run in parallel. For a more detailed walkthrough on how to parallelize the experiment, please see the `vignette("parallel")`.
...
Alternatively, `set_rmd_options()` can be used to update the R Markdown option for an existing object, rather than having to create the `Evaluator()` or `Plotter()` from scratch, e.g.,
```{r}
experiment <- experiment %>%
  set_rmd_options(field_name = "plotter", name = "Power", show = TRUE,
                  height = 10, width = 8)
```

To hide the output of an `Evaluator()` or `Plotter()` in the R Markdown report, set `show = FALSE` in `set_rmd_options()` or `create_*`.
...
For a concrete example of the saving structure, please refer to [this GitHub directory](https://github.com/Yu-Group/pcs.sim.pkg/tree/main/vignettes/results/Linear\ Regression\ Experiment/), where we have stored all saved results from this vignette.
