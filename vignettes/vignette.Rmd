...
experiment <- experiment %>%
  add_plotter(reject_prob_plot, name = "Rejection Prob. (alpha = 0.1) Plot")
...
#>    Methods: OLS 
#>    Evaluators: Rejection Prob. (alpha = 0.1) 
#>    Plotters: Power, Rejection Prob. (alpha = 0.1) Plot
#>    Vary Across: None
...
experiment <- experiment %>%
  update_plotter(power_plot, "Power")
```
...
Alternatively, `set_rmd_options()` can be used to update the R Markdown option for an existing object, rather than having to recreate the `Evaluator()` or `Plotter()` from scratch, e.g.,
```{r}
experiment <- experiment %>%
