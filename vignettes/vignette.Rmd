...
1. Create individual parts of the simulation experiment recipe
    ...
    d. Create Plotter(s) via `create_plot`
...
experiment <- create_experiment(name = "Experiment") %>%
  ...
  add_plot(plotter, name = "Plotter1")
```
...
To create a `DGP`, `Method`, `Evaluator`, or `Plotter`, we can respectively use the `create_dgp()`, `create_method()`, `create_evaluator()`, or `create_plot()` functions. Each `create_*` function follows the same syntax and takes in the inputs:
...
- First, the Evaluator function should almost always take in the named argument `fit_results`. `fit_results` is a placeholder for the tibble that will be outputted by `Experiment$fit()` and automatically passed in when the experiment is fitted. We will see `Experiment$fit()` in action later, but for now, we can think of `fit_results` as a tibble containing the results of all (repetition, DGP, method) combinations fitted in the experiment. Naturally, `fit_results` will have columns named `rep`, `dgp`, `method`, and any other named arguments that were outputted from the method function (i.e., `lm_fun`).
...
```{r}
power_plot <- create_plot(plot_fun = power_plot_fun)
```

Like the custom `Evaluator` functions, custom `Plotter` functions such as `power_plot_fun()` can take in the argument `fit_results` if we want to construct a plot from the method fit outputs. If we want to construct a plot using the output of our `Evaluators` (e.g., the output from `reject_prob_fun()`), the `Plotter` function can also take the argument `eval_results` as input. 

As with the other `create_*` functions, additional arguments that need to pass into the custom `Plotter` function can be passed into `create_plot()`.
...
experiment <- create_experiment(name = "Linear Regression Experiment") %>%
  ...
  add_plot(power_plot, name = "Power")
```
...
results <- experiment$run(n_reps = 100, save = T, verbose = 0)
...
Finally, we can easily visualize all results from the simulation experiment in an html file (generate using R Markdown) or browser.
...
In many situations, it is helpful to understand how a method's performance is affected as we vary a single parameter in the DGP across different values. For instance, what happens to the power as the features in `X` become more correlated?
...
In `add_vary_across`, the `dgp` argument is the name of the DGP to vary or the DGP object itself. The `param_name` argument is the name of the argument/parameter in the `DGP` function that will be varied, and the `param_values` argument is a list or atomic vector of values that `param_name` will take and vary across while all other arguments are kept constant at their base value (see `dgp$dgp_params`). Here, we are varying the `rho` parameter (i.e., the correlation) over values of 0, 0.2, 0.5, and 0.9 within the Linear Gaussian DGP. (Note: we can also vary across parameters in a `Method` by inputting the `method` argument instead of the `dgp` argument in `add_vary_across()`.)

However, when we run the experiment, the results are not quite what we expect. The results are summarized/aggregated across all values of `rho`.
...
To see how different values of `rho` affect the experiment results, we need to modify our `Evaluator` and `Plotter` functions. Specifically, in `reject_prob_eval`, we want to group `fit_results` by `rho` in addition to `dgp` and `method`. To do this, we need to add a `vary_param` argument to our custom `Evaluator` function. `vary_param` will be auto-filled by the `param_name` argument from `add_vary_across()` (in this case, `rho`) when we run the experiment. 
...
Similarly in `power_plot_fun`, we need to add a `vary_param` argument to plot the results across different values of `rho`.
...
vary_results <- experiment %>%
  ...
  update_plot(power_plot, name = "Power") %>%
  run_experiment(n_reps = 100, save = TRUE)
vary_results$eval_results
vary_results$plot_results
```
...
For fun, let's add another plot (in fact, an interactive plot using `plotly::ggplotly`) to our `Experiment` and run the `Experiment` across various values of the coefficient $\boldsymbol{\beta}_2$ and the noise SD $\sigma$. (Note: the plotter function below (`reject_prob_plot_fun`) takes in the `Evaluator` results, stored as `eval_results`, and plots the rejection probability for the $\boldsymbol{\beta}_1$ at $\alpha = 0.1$.)
...
reject_prob_plot <- create_plot(plot_fun = reject_prob_plot_fun, alpha = 0.1)
...
# run experiment across values of sigma
vary_sigma_results <- experiment %>%
  remove_vary_across() %>%
...
Notice in the above code block, we had to use `update_vary_across()` or `remove_vary_across()` + `add_vary_across()` instead of just `add_vary_across()`. This is because `experiment` already had a non-empty `vary_across` field. Another alternative to this and perhaps a more generally useful tool is to create a new experiment from another experiment through a deep clone.

```{r}
new_experiment <- create_experiment(name = "I am a clone",
...
vary_experiment <- experiment %>%
  add_vary_across(dgp = "Linear Gaussian DGP", param_name = "rho",
                  param_values = c(0, 0.2, 0.5, 0.9))
...
```{r}
fit_results <- experiment$fit(n_reps = 100, save = T)
...
```{r eval = FALSE}
fit_results <- fit_experiment(experiment, n_reps = 100, save = T)
eval_results <- evaluate_experiment(experiment, fit_results, save = T)
plot_results <- plot_experiment(experiment, fit_results, eval_results, save = T)
```
