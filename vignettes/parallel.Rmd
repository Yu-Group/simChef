...
method_fun <- function(X, y, sleep = 0.01) {
  fit <- lm(y ~ X)
  Sys.sleep(sleep)
  return(list(
    "p-val" = summary(fit)$coefficients["X2", "Pr(>|t|)"]
  ))
}

method1 <- create_method(method_fun)
method2 <- create_method(method_fun, sleep=0.1)
...
system.time(experiment$fit(n_reps = 100, parallel_strategy = c("reps", "dgps")))
