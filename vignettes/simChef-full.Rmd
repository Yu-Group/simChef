...
This creates a series of blank .md files for the user to fill out with descriptions of the simulation experiment and its recipe components. These blank .md files can be found in the experiment's root results directory under docs/. To find the experiment's root results directory, use `experiment$get_save_dir()`. You can find example .md files corresponding to the current experiment and others [here](https://github.com/Yu-Group/simChef/tree/main/vignettes/example-docs/).
...
3. `viz_results`: output of `visualize_experiment()`
    - A named list of elements, with one element for each `Visualizer` in the `Experiment`
      ...
      ```{r viz-results-output, fig.width = 4}
      results$viz_results
      ```
...
```{r vary-across-results-1, fig.width = 4}
...
```{r vary-across-results-2, fig.width = 6, fig.height = 6}
