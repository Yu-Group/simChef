---
title: "Getting started with simChef - Full Guide"
output:
...
## Setting global plotting theme for built-in `Visualizers`

By default, all `Visualizers` in the built-in `simChef` library use the default `ggplot` theme when plotting. To modify this plotting theme (for all built-in `Visualizers` and any `Visualizer` built upon `plot_eval_constructor()`), one can change the `simChef.plot_theme` global option.

In particular, to use the [vthemes](https://yu-group.github.io/vthemes/) plotting theme:

```{r eval = FALSE}
options(simChef.plot_theme = "vthemes")  # to use the vthemes plotting theme
```

More generally, one can specify any `ggplot::theme()` object.

```{r eval = FALSE}
options(simChef.plot_theme = ggplot2::theme_minimal())
```
